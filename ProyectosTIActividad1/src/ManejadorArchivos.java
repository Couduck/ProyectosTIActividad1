...
public class ManejadorArchivos      //Clase para manejar los archivos de HTML
{
    ...
    private Hashtable<String, Palabra> hashTablecentral = new Hashtable<>(10, 0.5f);    //Hashtable donde se almacenan las palabras

    private HashMap<String, Palabra> stopList = generarStopList();  //HashMap que contiene la StopList con los tokens que no se van a permitir
    ...
    //Se guarda el contenido del archivo de StopList en una HashMap
    ...
    //Se obtiene el peso de cada token en el archivo de Posting tiene dentro de su propio documento
    public void pesarTokens() throws IOException
    {
        ...
        BufferedReader bufredNumTokens = new BufferedReader(fileNumeroTokens), bufredPosting = new BufferedReader(filePosting); // BufferedReader que almacenan el documento del numero de tokens por archivo y el archivo Posting respectivamente
        String linea;  //Almacenará el archivo completo con el texto completo
        int contador = 1;
        HashMap<String, Integer> archivoVar = new HashMap<>();
        long inicioPesarToken, finPesarToken;

        //Se obtienen el total de tokens que hay por archivo y se coloca en un hashmap
        ...
        //Por cada linea del archivo de Posting se realiza lo siguiente
        while ((linea = bufredPosting.readLine()) != null)                               //Mientras no se haya llegado al final del archivo
        {
            ...
            //Se pesa el token y se redondea a 2 decimales
            double peso = Integer.parseInt(posting[1])*100.0/archivoVar.get(posting[0]);
            peso = peso*100;
            peso = Math.round(peso);
            peso = peso/100;

            //Se escribe el peso en el archivo correspondiente
            if(primeraVez)
            {
    ...
    //Se tokenizan todos los archivos, evitando guardar los que se encuentran en el documento de StopList.txt
    public void tokenizarTodosArchivosStopList() throws IOException     //Actividad 9
    {
        ...
        for(String pagina : files)
        {
            ...
            //Adquiere las palabras de los archivos utilizando el filtro de la StopList
        ...
        {
            //Los tokens que contengan que solo se repitan una vez o solamente sean de una letra se omiten de escribirse en el documento
    ...
    //Es algo muy similar a la tokenización de la parte pasada, solo que en vez de usar un HashMap, usa una Hashtable
    public void almacenamientoHashTable() throws IOException
    {
        ...
        for(String pagina : files)
        {
            ...
            //Se recuperan las palabras y se almacenan en una HashTable
        ...
        //Se obtienen las palabras de la HashTable
        ...
        //Se imprimen las palabras en el documento junto con su frecuencia y el hashcode que poseen
        ...
        //Se obtienen el total de colisiones al haber 2 palabras con el mismo hashcode y se coloca al final el total de colisiones en toda la tabla
    ...
    public void generarPosting() throws IOException
    {
        ...
        for(String pagina : files)
        {
            ...
            //Se recolectan las palabras de cada archivo y se almacenan en un HashCentral
            HashMap<String, Palabra> coleccionPalabrasArchivo = contarPalabrasXArchivo(pagina);
            AniadirAHashCentral(coleccionPalabrasArchivo, pagina);
        ...
        //El hash se pasa a un ArrayList
        Collection<Palabra> listaObjetos = hashcentral.values();
        ArrayList<Palabra> PalabrasComoLista = new ArrayList<>(listaObjetos);
        ...
        //Por cada palabra del listado se repite
        for(Palabra palabraCompleta : PalabrasComoLista)
        {
            int i = 0;

            /**Cada palabra guarda en 2 arreglos el numero del archivo en el que aparece y las repeticiones dentro de ese archivo. Ej:
             *
             * Perro aparece en 10 veces en 4 archivos
             *
             * Arreglo 1 de Perro = [102, 103, 89, simple]
             *
             * Arreglo 2 de Perro = [5,3,1,1]
             *
             * Ambos arreglos se relacionan al compartir indices:
             *
             * 1° Perro se repite 5 veces en archivo 102
             * 2° Perro se repite 3 veces en archivo 103
             * 3° Perro se repite 1 vez en archivo 89
             * 4° Perro se repite 1 vez en archivo 1
             */
...
//
    ...
    //Se cuentan todos los tokens por cada diccionario
    ...
    //Filtra las palabras de un archivo en base a StopList.txt
...
//        }
    ...
    //Filtro que analiza si la palabra se encuentra en el StopList
